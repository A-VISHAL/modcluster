...
import { createHandoverCard, saveHandover } from '../core/handover';
...
// Handover form submission endpoint
...
forms.post('/handover-submit', async (c) => {
  const values = await c.req.json<HandoverFormValues>();
  const subId = context.subredditId || 'global';
  const author = context.username || 'moderator';

  const handover = createHandoverCard({
    author,
    activeSituations: values.activeSituations || '',
    usersToWatch: values.usersToWatch || '',
    priorityPosts: values.priorityPosts || '',
    notes: values.notes || '',
  });

  try {
    await saveHandover(subId, handover);
    return c.json(
      {
        showToast: 'Handover saved and set as active.',
      },
      200
    );
  } catch (err) {
    ...
    return c.json(
      {
        showToast: 'Failed to save handover. Please try again.',
      },
      200
    );
  }
